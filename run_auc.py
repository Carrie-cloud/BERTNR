...
def main():
    ...
    parser.add_argument("--output_dir",
                        default='output/auc/kg-bert_auc_fb237_v3-200',
                        type=str,
                        required=False,
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # 转换logits为概率
        probs = torch.softmax(torch.tensor(preds), dim=1)[:, 1].numpy()  # 假设正类标签在索引1
        all_label_ids = all_label_ids.numpy()

        # 计算AUC-PR
        precision, recall, _ = precision_recall_curve(all_label_ids, probs)
